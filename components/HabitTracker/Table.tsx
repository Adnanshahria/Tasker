...
import { Trash2, Check, X, HelpCircle, FileText, Settings2, GripVertical } from 'lucide-react';
...
const ReorderModal: React.FC<{
    ...
    onClose: () => void;
}> = ({ isOpen, habits, onSave, onClose }) => {
    const [tempHabits, setTempHabits] = useState<LocalHabit[]>(habits);
    ...
    const handleDrop = (e: React.DragEvent, targetId: string) => {
        ...
        if (draggedId && draggedId !== targetId) {
            const sourceIdx = tempHabits.findIndex(h => h.id === draggedId);
            const targetIdx = tempHabits.findIndex(h => h.id === targetId);
            const newHabits = [...tempHabits];
            newHabits.splice(sourceIdx, 1);
            newHabits.splice(targetIdx, 0, tempHabits[sourceIdx]);
            ...
            >
                <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-bold text-white">Reorder Habits</h3>
                    </div>
                ...
                <p className="text-sm text-slate-400 mb-4">Drag habits to reorder them:</p>
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                    {tempHabits.map((habit, idx) => (
                        <div
                            key={habit.id}
                            draggable
                            ...
                            className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-grab active:cursor-grabbing
                                ${draggedId === habit.id ? 'opacity-50 bg-slate-700 border-emerald-500/50' : 'bg-slate-800/50 border-slate-700/50 hover:border-emerald-500/30'}
                                ${dragOverId === habit.id ? 'border-t-2 border-t-emerald-400' : ''}`}
                        >
                            <GripVertical size={16} className="text-slate-500" />
                            <span className="text-slate-200 font-medium flex-1 truncate">{habit.title}</span>
                ...
                <div className="flex gap-3 mt-4">
                    ...
                    <button onClick={() => { onSave(tempHabits.map(h => h.id)); onClose(); }} className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium transition-colors flex items-center justify-center gap-2">
        ...
        <>
            <table className="w-full text-left border-separate border-spacing-0">
                <thead className="sticky top-0 z-10">
                    <tr className="bg-gradient-to-r from-slate-800 to-slate-800/90 backdrop-blur-sm text-slate-300 text-[10px] md:text-sm font-bold border-b-2 border-emerald-500/30">
                        <th className="w-8 md:w-10 p-2 md:p-3 text-center text-slate-400 bg-slate-800 border-b border-r border-slate-600/50 sticky left-0 z-20">
                            <button onClick={() => setShowReorderModal(true)} className="p-1 hover:bg-emerald-500/20 rounded transition-colors" title="Reorder habits">
                                <Settings2 size={12} className="text-emerald-400" />
                            </button>
                        </th>
                        <th className="p-2 md:p-3 border-b border-r border-slate-600/50 w-[100px] md:w-[160px] bg-slate-800 sticky left-8 md:left-10 z-20">{t.habitName}</th>
                        {daysInMonth.map(day => (
                            <th key={day.toISOString()} className={`p-0.5 md:p-1 border-r border-slate-600/50 w-6 md:w-8 text-center text-[10px] md:text-xs ${isSameDay(day, new Date()) && isSameMonth(day, new Date()) ? 'bg-emerald-500/30 text-emerald-300' : ''}`}>
                                {format(day, 'd')}
                            </th>
                        ))}
                    ...
                    <AnimatePresence>
                        {habits.map((habit, idx) => (
                            <motion.tr
                                ...
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="group transition-colors border-b border-slate-700/50 hover:bg-emerald-500/5"
                            >
                                {/* Serial Number */}
                                <td className="w-8 md:w-10 text-center text-[10px] md:text-xs text-slate-400 bg-slate-800 font-mono p-1.5 md:p-2 border-b border-r border-slate-700/50 sticky left-0 z-10">
                                    {idx + 1}
                                </td>
                                <td className="p-0 border-b border-r border-slate-700/50 bg-slate-900 sticky left-8 md:left-10 z-10">
                                    <div className="px-1.5 md:px-2 py-1.5 md:py-2 flex items-center gap-1">
                                        <span className="text-slate-100 font-semibold truncate text-xs md:text-sm flex-1">{habit.title}</span>
                                        <button
                                            onClick={() => openDescModal(habit)}
                                            className={`p-0.5 md:p-1 transition-colors flex-shrink-0 rounded ${habit.description
                                ...
                                {daysInMonth.map(day => {
                                    ...
                                    return (
                                        <td key={day.toISOString()} className="p-0 border-r border-slate-700/50 text-center">
                                            <button onClick={() => onToggle(habit, day)} className={`w-full h-8 md:h-10 flex items-center justify-center transition-all ${done ? 'bg-emerald-500/30 text-emerald-400' : missed ? 'bg-red-500/10 text-red-400/60' : 'hover:bg-slate-800'}`}>
