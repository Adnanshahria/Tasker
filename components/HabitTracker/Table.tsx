    ...
    <table className="w-full text-left border-collapse">
        <thead className="sticky top-0 z-10">
            <tr className="bg-gradient-to-r from-slate-800 to-slate-800/90 backdrop-blur-sm text-slate-300 text-[10px] md:text-xs font-bold border-b-2 border-emerald-500/30">
                ...
                <th className="p-2 md:p-3 border-r border-slate-600/50 w-[90px] md:w-[120px] sticky left-[32px] z-20 bg-slate-800">{t.habitName}</th>
            ...
            <AnimatePresence>
                {habits.map((habit, idx) => (
                    <motion.tr key={habit.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group hover:bg-emerald-500/5 transition-colors border-b border-slate-700/50">
                        ...
                        <td className="p-0 border-r border-slate-700/50 sticky left-[32px] z-10 bg-slate-900"><div className="px-1.5 md:px-2 py-1.5 md:py-2 text-slate-200 font-medium truncate text-xs md:text-sm">{habit.title}</div></td>
