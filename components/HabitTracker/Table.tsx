    ...
    <table className="w-full text-left border-separate border-spacing-0">
        <thead className="sticky top-0 z-10">
            <tr className="bg-gradient-to-r from-slate-800 to-slate-800/90 backdrop-blur-sm text-slate-300 text-[10px] md:text-xs font-bold border-b-2 border-emerald-500/30">
                <th className="w-8 md:w-10 p-2 md:p-3 text-center text-slate-500 bg-slate-800 border-b border-r border-slate-600/50 sticky left-0 z-20">#</th>
                <th className="p-2 md:p-3 border-b border-r border-slate-600/50 w-[90px] md:w-[120px] bg-slate-800 sticky left-8 md:left-10 z-20">{t.habitName}</th>
            ...
            <AnimatePresence>
                {habits.map((habit, idx) => (
                    <motion.tr key={habit.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group hover:bg-emerald-500/5 transition-colors border-b border-slate-700/50">
                        <td className="w-8 md:w-10 text-center text-[10px] md:text-xs text-slate-500 bg-slate-800 font-mono p-1 md:p-2 border-b border-r border-slate-700/50 sticky left-0 z-10">{idx + 1}</td>
                        <td className="p-0 border-b border-r border-slate-700/50 bg-slate-900 sticky left-8 md:left-10 z-10"><div className="px-1.5 md:px-2 py-1.5 md:py-2 text-slate-200 font-medium truncate text-xs md:text-sm">{habit.title}</div></td>
